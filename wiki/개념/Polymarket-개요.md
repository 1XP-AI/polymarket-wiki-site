...
## 예제: Polymarket API 호출

다음 예제는 Polymarket의 공개 API(감마 API)를 사용해 이벤트/마켓 데이터를 조회하는 간단한 예시입니다. 실제 응답 구조는 API 버전 및 엔드포인트에 따라 달라질 수 있으니, 자세한 스키마는 [[Polymarket-API-레퍼런스]]를 확인하세요.
...
- Python (requests) 예제: 최근 마켓 제목을 출력
...
마켓에서 표시되는 가격(price)은 해당 결과의 암묵적 확률을 의미합니다. 예를 들어 가격이 0.72이면 약 72%의 확률을 의미한다고 해석할 수 있습니다(수수료/슬리피지 고려 전).
