...
Polymarket은 사건 결과에 베팅하는 예측시장(prediction market) 플랫폼입니다. 각 마켓은 Yes/No 또는 다중 아웃컴(outcome)을 가지며, 가격은 해당 결과의 암묵적 확률을 반영합니다. 온체인/오프체인 혼합 구조를 사용하고, 마켓 토큰은 CTF(Conditional Token Framework)로 표현될 수 있습니다(예: [[개념/CTF]]).
...
- 대상 선정: 리더보드·포지션·승률 등 계량지표로 후보를 선별하되, 과거 성과의 표본오차와 조작 가능성을 고려(자세한 선별 기준은 [[개념/Copytrade]] 참조).
...
- 비용 관찰: 실거래 전 시뮬레이션(백테스트)으로 누적 비용을 포함한 기대 수익률을 검증(관련: [[개념/Copytrade#백테스트 설계 (권장)]]).
- 리스크 완화: 최대 익스포저 한도, 추종 중단 규칙(예: 연속 손실 n회 시 정지), 포지션 캡 등 자동화 규칙 적용(관련: [[전략/Copytrade/추종-중단-규칙]], [[전략/Copytrade/리스크-관리]]).
- 모니터링: 리더보드 조작 탐지, 오라클 지연/결정 오류 모니터링, 비정상 트랜잭션 감지(관련: [[전략/Copytrade/리더보드-조작-탐지]], [[전략/Copytrade/리스크-시나리오]]).
...
| 토큰 | 개별 Yes/No 토큰 | 조건부 토큰(ERC-1155) |
| 변환 | No 토큰 ↔ Yes 토큰 + USDC (통합) | Split: $1 → Yes-A + Yes-B + Yes-C |
| API 파라미터 | `negRisk: true` 필요 | 일반 CLOB 주문 |
| 복잡도 | 낮음 (여러 독립 마켓) | 높음 (토큰 조합 필요) |

### Neg-Risk 컨트랙트 구조

- **NegRiskAdapter 컨트랙트**: `github.com/Polymarket/neg-risk-ctf-adapter` — 상호 배타적 바이너리 마켓을 단일 다중 결과 구조로 통합
- **기본 원리**: `1 Yes + 1 No = 1 USDC` 수학적 보장 — 포지션 운영과 마켓 무결성의 기초
- **No 변환**: 여러 No 포지션을 보유 = 모든 결과에 Yes + USDC 보유와 동등

### Neg-Risk 거래량

Polymarket의 Neg-Risk 거래량은 $14억 이상 처리되었으며, CTF 마켓을 추월하는 추세입니다(CryptoSlate 2026). 일일 활성 사용자 8,600명 기록.

### Copytrade 관점

- Neg-Risk 마켓은 CTF 대비 진입 장벽이 낮아 초보 카피 트레이더에게 적합
- 주문 제출 시 반드시 `negRisk: true` 파라미터 필요 ([[기술/API/Polymarket-API-레퍼런스]])
- 정치 카테고리에서 빈번하게 발생 ([[마켓/정치]])

(자세한 수학적 구조: [[개념/Negative-Risk]])
...
- [[개념/CTF]] — 마켓의 온체인 토큰 표현
- [[개념/CLOB]] — 주문장 기반 거래와 가격 형성
- [[전략/Copytrade/자동화-플로우]] — 추종 전략과 성과 비교
- [[트레이더/리더보드/데이터]] — 트레이더 데이터 수집 및 지표
