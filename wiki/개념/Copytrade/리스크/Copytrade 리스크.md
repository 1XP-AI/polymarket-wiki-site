...
<!-- para: para_001 -->
> Copytrade 운영 리스크를 묶어 보는 하위 허브 문서

<!-- para: para_002 -->
실행 비용, 성과 분석, 모니터링 문서를 분리해 위험 신호와 대응 지점을 빠르게 찾게 한다.
...
<!-- para: para_004 -->
핵심 리스크 분석 문서군: [[Copytrade 리스크 관리]], [[Copytrade 성능 모니터링]], [[딜레이(지연) 분석 및 슬리피지 대응]], [[리스크 모니터링(Copytrade)]], [[리스크 시나리오]], [[리스크 알림 규칙]], [[성과 분석 지표 (Copytrade 성능 메트릭)]]

## 탐색 경로

<!-- para: para_005 -->
실행 전에 `Copytrade 리스크 관리 -> 실행 비용(Execution Cost) — Copytrade 고려사항 -> 리스크 모니터링(Copytrade)` 순으로 읽는 편이 안전하다.
