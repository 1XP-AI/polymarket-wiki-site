...
- Polymarket Copytrade에서 플랫폼 수수료, 슬리피지, 시장 충격을 합산한 총 실행 비용을 마켓별로 어떻게 추정하고, 포지션 크기별 손익분기점까지 계산할 수 있는가?
- 유동성이 낮은 Polymarket 마켓에서 주문량 증가에 따라 슬리피지와 시장 충격이 비선형적으로 커지는 구간은 어떻게 식별하고, Kyle's lambda를 실무적으로 어떻게 추정할 수 있는가?
- 리더 주문과 팔로워 주문 사이의 지연 시간 3초 이상 구간에서 슬리피지가 50% 이상 증가하는 사례를 어떻게 검증하고, 자동화 Copytrade의 큐잉 전략을 어떻게 설계해야 하는가?
