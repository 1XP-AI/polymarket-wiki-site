...
- Copytrade에서 원 트레이더의 수익률보다 먼저 점검해야 하는 추종 대상 리스크 지표는 무엇이며, 리더보드 상위 계정 중에서도 장기 팔로우를 피해야 할 패턴은 어떻게 식별할 수 있는가?
- 유동성이 얕은 마켓에서 슬리피지와 체결 지연이 기대수익을 얼마나 훼손하는지 정량화하려면 어떤 테스트나 로그 수집이 필요한가?
- 계정 규모가 다른 상황에서 단일 트레이더 최대 노출과 단일 이벤트군 최대 노출을 어떤 기준으로 산정해야 하며, 비중 축소 규칙은 어떻게 설계하는 것이 적절한가?
