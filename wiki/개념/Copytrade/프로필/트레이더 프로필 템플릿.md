---
id: legacy_wiki_88f80c
title: 트레이더 프로필 템플릿
type: concept
status: verified
created_at: '2026-04-09T14:10:12Z'
last_updated: '2026-04-11T16:12:37Z'
as_of: '2026-04-09'
owners:
- wiki-system
source_count: 1
evidence_coverage: 1.0
confidence: medium
related_pages:
- concept_copytrade
- legacy_wiki_copytrade_4d0fc3
- legacy_wiki_copytrade_698574
- legacy_wiki_copytrade_bb99a4
tags:
- concept
sources:
- url: raw/fetched/2026-04-05-leaderboard-monthly.md
cluster: Copytrade
cluster_group: 프로필
doc_role: template
---
# 트레이더 프로필 템플릿

## 요약

<!-- para: para_001 -->
> 한 줄 요약: 리더보드에서 발견한 트레이더를 분석할 때 사용하는 표준 프로필 템플릿

<!-- para: para_002 -->
- 이름/핸들: 
- 지갑주소: 
- 활동 기간: 
- 추종 권고 (예/아니오) 및 가중치:

<!-- para: para_003 -->
트레이더의 공개 소셜(트위터/X 등), 지갑주소, 첫 활동일 등을 기록한다. 가능하면 리더보드 내 순위 추이와 주요 포지션 히스토리를 포함한다.

<!-- para: para_004 -->
- 리더보드는 트레이더의 성과 지표(수익률, 거래 빈도 등)를 제공한다. Copytrade 선별 시 다음 항목을 반드시 확인한다:
  - 일관성(기간별 승률)
  - 포지션 크기 및 마켓 선호도
  - 공개 소셜(트윗)과의 일치 여부

<!-- para: para_005 -->
- 선호 마켓(스포츠/크립토/정치 등)
- 평균 포지션 보유 시간
- 레버리지/포지션 규모 특성
- 자주 사용하는 이벤트 유형

<!-- para: para_006 -->
- 총 수익률(가능하면 기간별)
- 승률 / 평균 수익-손실 비율
- 최대 드로우다운

<!-- para: para_007 -->
- 과거 급격한 포지션 변경 빈도
- 다른 트레이더와의 포지션 유사도 (군집성)
- 의심스러운 패턴(조작 가능성) 유무

## 열린 질문

- 리더보드에서 특정 트레이더의 수익률, 승률, 최대 드로우다운을 기간별로 확인해 일관성이 높은지 검증하려면 어떤 지표 조합을 우선 조사해야 하는가?
- 트레이더의 공개 소셜(X/트위터) 활동과 실제 지갑 거래 기록이 얼마나 일치하는지 검증하려면 어떤 시점의 게시물과 어떤 포지션 변화를 대조해야 하는가?
- 해당 트레이더가 주로 거래하는 마켓 유형(스포츠, 크립토, 정치 등)과 평균 포지션 보유 시간, 포지션 규모를 함께 분석했을 때 Copytrade 대상 적합성이 어떻게 달라지는가?
