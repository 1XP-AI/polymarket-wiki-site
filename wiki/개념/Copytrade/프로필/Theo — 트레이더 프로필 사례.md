...
- Theo의 월간 리더보드 순위와 누적 PnL이 실제로 어떤 기간 동안 유지됐는지, 2026-04-09 기준 최신 leaderboard API와 Polymarket Analytics에서 교차 검증해볼 수 있는가?
- Theo의 거래 히스토리에서 이벤트 직전 진입 비중이 얼마나 되는지, 정치·크립토·거시 마켓별로 early entry 패턴을 정량화할 수 있는가?
- Theo가 주당 5-15회 수준의 거래 빈도를 유지하면서 평균 승률 55-65%를 기록하는지, 표본 수와 기간을 통제해 재현 가능한지 확인할 수 있는가?
