...
type: concept
status: verified
created_at: '2026-04-11T11:45:36Z'
...
<!-- para: para_001 -->
> 리더보드 기반 후보 발굴과 선별 기준을 묶는 하위 허브 문서

<!-- para: para_002 -->
리더보드 군집은 데이터 참조, 후보 선별, 분석, 조작 탐지를 역할별로 나눠 읽을 때 가장 이해가 쉽다.
...
<!-- para: para_004 -->
데이터 참조 문서군: [[데이터 기반 트레이더 선별]], [[리더보드 & 트레이더 데이터]], [[리더보드 데이터]]

## 분석/선별

<!-- para: para_005 -->
분석/선별 문서군: [[Copytrade 리더보드 심화]], [[Copytrade 알파 지표 선별]], [[리더보드 분석]], [[리더보드 알파 탐지]], [[리더보드 조작 탐지]]

## 탐색 경로

<!-- para: para_006 -->
처음에는 `리더보드 데이터 -> 리더보드 & 트레이더 데이터 -> 데이터 기반 트레이더 선별 -> 리더보드 분석` 순서로 읽는 편이 자연스럽다.
