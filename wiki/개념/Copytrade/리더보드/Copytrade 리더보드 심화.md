...
- 공식 Polymarket 리더보드와 공식 프로필 데이터만으로 추종 후보를 거르기 위한 최소 필수 지표는 무엇이며, 각 지표의 권장 임계값은 어떻게 설정할 수 있는가?
- 거래량 대비 수익 비율이 높은 계정과 거래량이 크지만 수익이 완만한 계정을 구분할 때, 어떤 패턴이 일시적 대형 적중형이고 어떤 패턴이 반복 가능한 전략형인지 검증할 수 있는가?
- 상위 리더보드 계정들을 수익 규모, 활동 지속성, 포지션 집중도, 손실 이후 회복력 기준으로 비교했을 때, copytrade 추종 후보로 가장 안정적인 조합은 어떻게 선별되는가?
