...
- Polymarket copytrade에서 리더보드 상위 트레이더의 단순 수익률 대신 보유 기간, 반복 진입 패턴, 손실 후 회복 속도를 함께 반영한 추종 점수 모델을 어떻게 설계하고 검증할 수 있는가?
- 내 계정 규모와 Polymarket 시장 유동성, 스프레드, 슬리피지를 기준으로 트레이더 포지션을 어떤 규칙으로 축소·분산해야 재현 가능한 copytrade가 되는가?
- copytrade 후보를 실제 주문 단계로 넘기지 않기 위한 최소 진입 기준은 무엇이며, 공개 프로필과 최근 체결 기록에서 어떤 신호를 우선적으로 제외 조건으로 써야 하는가?
