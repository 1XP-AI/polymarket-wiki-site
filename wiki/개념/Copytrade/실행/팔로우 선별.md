...
- concept
sources:
- url: https://docs.revealdata.com/docs/2026-2-3-30-patch-release-notes-reveal
cluster: Copytrade
cluster_group: 실행
doc_role: playbook
---
...
<!-- para: para_005 -->
- [[Copytrade 데이터 수집 가이드]] — 트레이더 성향 분석 방법론
- [[Copytrade 데이터 수집 가이드]] — 포지션 규모와 손실제한 규칙
- [[Copytrade 데이터 수집 가이드]] — 주문 배분과 리밸런싱 가이드
- [[Copytrade 데이터 수집 가이드]] — 리더보드 기반 알파 탐지 방법론
...
- 최근 90일 수익곡선, 최대 드로다운, 평균 포지션 보유시간, 거래 빈도를 함께 기준으로 삼아 Copytrade 팔로우 대상을 정량 선별하는 구체적 스코어링 방법은 무엇인가?
- 리더보드 상위 트레이더와 실제 복제 성과가 어긋나는 사례를 찾기 위해, 트레이더 프로필과 과거 포지션 로그에서 어떤 반복 패턴과 집중 위험 신호를 점검해야 하는가?
- 고빈도 복제 대상인 Theo처럼 거래 빈도가 높은 트레이더를 팔로우할 때, 포지션 분할과 손실 임계값 기반 자동 언팔로우 규칙은 어떤 조건으로 설계하는 것이 적절한가?
