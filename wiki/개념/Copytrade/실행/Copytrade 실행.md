...
<!-- para: para_001 -->
> Copytrade 실행/운영 절차를 모아 놓은 하위 허브 문서

<!-- para: para_002 -->
실행 군집은 데이터 수집, 신호 필터링, 자동화, 리밸런싱 같은 playbook 문서를 하나의 흐름으로 묶는다.
...
<!-- para: para_004 -->
실행 가이드 문서군: [[Copytrade 데이터 수집 가이드]], [[Copytrade 자동화 플로우]], [[Copytrade 포지션 리밸런싱 가이드]], [[추종 신호 필터링]], [[추종 실행 속도 최적화]], [[추종 중단 규칙]], [[팔로우 선별]], [[팔로우 스코어링 모델]]

## 운영 분석

<!-- para: para_005 -->
운영 분석 문서군: [[Polymarket Copytrade 전략 분석]], [[실행 비용(Execution Cost) — Copytrade 고려사항]]

## 참고 데이터

<!-- para: para_006 -->
참고 데이터 문서군: [[데이터 수집 파이프라인]]

## 탐색 경로

<!-- para: para_007 -->
운영 절차를 잡을 때는 `Copytrade 데이터 수집 가이드 -> 팔로우 선별 -> Copytrade 자동화 플로우 -> Copytrade 포지션 리밸런싱 가이드` 순서가 적합하다.
