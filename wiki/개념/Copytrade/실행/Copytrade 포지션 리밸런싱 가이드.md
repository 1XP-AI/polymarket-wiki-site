...
- Polymarket copytrade에서 리밸런싱 트리거를 일일 재계산, 변동성 급등, 주문장 깊이 급감, 이벤트 24시간 전 축소로 나눌 때 각 트리거별 권장 기준값과 우선순위는 무엇인가?
- Polymarket 시장에서 포지션 상한을 계정 자본의 5~10%로 둘 때 슬리피지, 체결 실패, 수수료를 포함한 실제 리스크 기준으로 이 범위를 어떻게 보정해야 하는가?
- copytrade 대상 트레이더의 포지션이 급격히 n배 확대 또는 축소될 때 선행 리밸런싱과 후행 리밸런싱 중 어떤 조건에서 각각 더 유리한가?
