...
<!-- para: para_005 -->
트레이더 선별 단계에서는 단순 수익률보다 승률, 최대 드로우다운, 평균 보유 시간, 거래 빈도, 특정 이벤트군 편중 여부를 함께 봐야 한다. 리더보드 상위권이라도 특정 시즌성 마켓에만 강한 계정은 자동 복제 대상으로 부적합할 수 있다.
...
<!-- para: para_007 -->
실제 구현에서는 WebSocket만으로 충분하지 않다. 체결 스트림이 끊길 수 있으므로 주기적 포지션 스냅샷과 온체인 데이터로 재조정해야 하며, Neg-Risk 마켓이나 유동성이 얕은 마켓은 일반 CLOB 마켓과 별도 규칙으로 다루는 편이 안전하다.

<!-- para: para_008 -->
운영 체크리스트는 손실 한도 초과 시 복제 중단, API 오류 누적 시 안전 정지, 대상 트레이더의 전략 붕괴 감지 시 제외, 슬리피지 급증 구간 회피, 그리고 사람이 검토할 수 있는 알림과 로그를 남기는 구조를 먼저 갖추는 것이다.

## 실무 포인트

<!-- para: para_009 -->
실무에서는 "복제 엔진"과 "평가 엔진"을 분리하는 편이 낫다. 평가 엔진은 어떤 트레이더를 따라갈지와 비중을 계산하고, 복제 엔진은 실제 주문 전송과 재동기화, 장애 복구를 담당한다. 이렇게 분리해야 특정 트레이더를 교체해도 실행 계층을 다시 짜지 않아도 된다.

<!-- para: para_010 -->
Copytrade 자동화의 성패는 체결 속도보다 예외 처리에서 갈린다. 리더보드가 좋아 보여도 급격한 노출 확대, 장중 유동성 붕괴, 시장 상태 전환, API 장애가 겹치면 복제 계정의 손실이 훨씬 커질 수 있다. 따라서 자동화 플로우에는 사람 검토용 로그와 서킷 브레이커가 반드시 포함돼야 한다.
