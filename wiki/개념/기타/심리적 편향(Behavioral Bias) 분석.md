...
- Polymarket 리더보드 상위 트레이더의 과거 거래 데이터에서 손실회피 편향을 정량화할 때, "손실 구간 진입 후 마감 24시간 전까지 미청산 비율 60%+" 기준이 실제로 얼마나 유효한지 검증할 수 있는가?
- 평균 보유기간, 포지션 회전율, 최근 10건 승률 편차, 평균 손절 시점 같은 지표를 결합했을 때, 특정 트레이더의 편향 점수 25+가 향후 수익률 하락이나 최대낙폭 확대를 얼마나 잘 예측하는가?
- 확증 편향이 강한 트레이더는 어떤 온체인 행동 패턴으로 식별되는가? 예를 들어 같은 시장에 반복적으로 재진입하거나 반대 신호 발생 후에도 포지션을 유지하는 패턴이 실제로 관찰되는지 조사할 수 있는가?
