...
- Polymarket Copytrade에서 리더보드 신규 포지션 신호와 최근 24시간 거래량 급증을 결합했을 때, 진입 후 수익률과 미체결률이 어떻게 달라지는지 6개월 이상 데이터로 백테스트할 수 있는가?
- 신호 발생부터 주문 체결까지의 실행지연(latency)이 슬리피지와 최종 수익률에 미치는 영향을 정량화할 때, 어떤 분위수 기준(예: 50/90/95퍼센타일)이 가장 유의미한가?
- 리더보드 활동, 이벤트 API, 외부 뉴스 신호를 각각 단독으로 사용할 때와 점수화해 결합할 때의 복제성공률, 샤프, MDD 차이는 어느 정도인가?
