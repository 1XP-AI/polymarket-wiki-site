...
<!-- para: para_008 -->
이 명령은 `gamma-api.polymarket.com`에서 지정한 `MARKET_ID`의 최근 체결 내역을 가져온 뒤, 각 거래에서 `price`, `size`, `timestamp`만 뽑아 보기 쉽게 정리합니다.
...
- Copytrade에서 리드 트레이더의 주문 크기와 추종자 동시 진입 비율이 시장 충격(impact cost, PRI)에 어떤 상관관계를 보이는지, 실제 체결 로그로 구간별 분석할 수 있는가?
- `gamma-api.polymarket.com`의 `trades`와 호가 데이터로 주문 전 Mid Price, 체결가, N분 후 가격을 연결해 시장 충격과 복구 속도(PRI)를 자동 계산하는 검증 가능한 방법은 무엇인가?
- 유동성이 얇은 마켓과 두꺼운 마켓에서 동일한 Copytrade 신호가 발생했을 때, Best Bid/Ask 확장 폭과 가격 복구 패턴이 어떻게 달라지는지 비교할 수 있는가?
