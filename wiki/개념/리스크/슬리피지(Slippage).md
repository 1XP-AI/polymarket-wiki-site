...
<!-- para: para_005 -->
- 포지션 크기 제한: [[Copytrade 데이터 수집 가이드]]의 권장 비율에 따라 원 트레이더 주문 대비 작은 비율로 진입한다. - 슬리피지 한도 설정: 예상 최대 슬리피지 이상이면 진입 취소(리미트 또는 스프레드 기반 진입). - 유동성 고려: 마켓의 호가 깊이와 최근 체결량을 모니터링하여 대형 주문을 회피.
