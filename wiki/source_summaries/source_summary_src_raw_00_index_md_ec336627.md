---
id: source_summary_src_raw_00_index_md_ec336627
title: 'Source Summary: Polymarket Copytrade 위키 — Raw 자료 인덱스'
type: source_summary
status: verified
created_at: '2026-04-09T14:10:08Z'
last_updated: '2026-04-09T14:10:08Z'
as_of: '2026-04-09'
owners:
- wiki-system
source_count: 1
evidence_coverage: 1.0
confidence: high
related_pages:
- legacy_raw_00_index_ec3366
tags:
- internal
- internal
---

# Source Summary: Polymarket Copytrade 위키 — Raw 자료 인덱스

## Summary

<!-- para: para_001 -->
> 생성일: 2026-04-04
> 수집 대상: Polymarket 공식 문서 (docs.polymarket.com) + Signal Foundry 설계 문서

## Key Facts

<!-- para: para_002 -->
| # | 파일명 | 내용 | 상태 |
|---|--------|------|------|
| 01 | [[01-Polymarket-개요]] | Polymarket 플랫폼, 핵심 개념, API 구조 | ✅ 완료 |
| 02 | [[02-API-레퍼런스]] | 전체 API 엔드포인트 목록 (Events, Markets, Orders, Trades 등) | ✅ 완료 |
| 03 | [[03-Copytrade-전략분석]] | Copytrade 파이프라인, 트레이더 선별, 리스크 관리 | ✅ 완료 |
| 04 | [[04-마켓메이커-리베이트]] | Maker Rebates, Liquidity Rewards, 인벤토리 관리 | ✅ 완료 |
| 05 | [[05-리더보드-트레이더-데이터]] | 리더보드, 프로필, 포지션, 거래 내역 API | ✅ 완료 |
| 06 | [[06-WebSocket-실시간데이터]] | WebSocket 채널, 실시간 감지 방법 | ✅ 완료 |
| 07 | [[07-온체인-컨트랙트]] | 컨트랙트, Subgraph, 블록체인 데이터 | ✅ 완료 |

<!-- para: para_003 -->
- [ ] Polymarket Help Center (사용자 가이드)
- [ ] 커뮤니티 copytrade 전략 글
- [ ] 경쟁 플랫폼 비교 (Azuro, Overtime 등)
- [ ] 실제 리더보드 상위 트레이더 데이터 스냅샷
- [ ] Negative Risk 마켓 상세 분석

## Details

<!-- para: para_004 -->
이 raw 자료를 LM Studio (Gemma 4 26B)에 넣고 → 한글 위키로 컴파일

## Open Questions

<!-- para: para_005 -->
No open questions extracted.
