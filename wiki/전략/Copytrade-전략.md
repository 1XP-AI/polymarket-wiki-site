...
> 리더보드 기반 카피트레이드의 주요 전략과 리스크 관리 방안. Copytrade의 개념적 기반은 [[개념/Copytrade]] 문서를 참조.

## 내용

성공적인 카피트레이드는 트레이더 선정(승률, 위험 프로필), 포지션 스케일링, 진입/청산 규칙, [[Copytrade-리스크-관리|손절(스톱로스) 전략]]을 포함합니다. [[리더보드-트레이더-데이터|리더보드의 빈도(거래 빈도)]]와 포지션 사이즈를 분석하여 자동 복제 비율을 결정해야 합니다. 또한 [[개념-CLOB|CLOB 기반 주문집행]]의 [[슬리피지|슬리피지]]와 수수료를 시뮬레이션하는 것이 필수입니다.
...
- [[전략/Copytrade/전략분석]] — CTF가 전략에 미치는 영향
