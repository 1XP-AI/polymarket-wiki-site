...
이들 지표는 raw/fetched API(예: Polymarket 이벤트/markets 엔드포인트)를 주기적으로 폴링하여 산출한다. 데이터 수집은 매 5분~1시간 주기(전략 민감도에 따라 조정)로 수행하며, 급변 시(예: 시장 충격)에는 실시간 WebSocket 연동을 권장한다[[Polymarket-개요]]
